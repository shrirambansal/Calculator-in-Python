from tkinter import *

expression = ""


def insert(num):

  global expression

  expression = expression + str(num)

  equation.set(expression)


def equal_insert():

  try:

    global expression

    total = str(eval(expression))

    equation.set(total)

    expression = ""

  except:

    equation.set(" error ")
    expression = ""


def clear():
  global expression
  expression = ""
  equation.set("")


if __name__ == "__main__":

  gui = Tk()

  gui.configure(background="white")

  gui.title("Calculator By SRB")

  gui.geometry("340x170")

  equation = StringVar()

  expression_field = Entry(gui, textvariable=equation)

  expression_field.grid(columnspan=4, ipadx=70)

  button1 = Button(gui,
                   text=' 1 ',
                   fg='white',
                   bg='silver',
                   command=lambda: insert(1),
                   height=1,
                   width=7)
  button1.grid(row=5, column=0)

  button2 = Button(gui,
                   text=' 2 ',
                   fg='white',
                   bg='silver',
                   command=lambda: insert(2),
                   height=1,
                   width=7)
  button2.grid(row=5, column=1)

  button3 = Button(gui,
                   text=' 3 ',
                   fg='white',
                   bg='silver',
                   command=lambda: insert(3),
                   height=1,
                   width=7)
  button3.grid(row=5, column=2)

  button4 = Button(gui,
                   text=' 4 ',
                   fg='white',
                   bg='silver',
                   command=lambda: insert(4),
                   height=1,
                   width=7)
  button4.grid(row=4, column=0)

  button5 = Button(gui,
                   text=' 5 ',
                   fg='white',
                   bg='silver',
                   command=lambda: insert(5),
                   height=1,
                   width=7)
  button5.grid(row=4, column=1)

  button6 = Button(gui,
                   text=' 6 ',
                   fg='white',
                   bg='silver',
                   command=lambda: insert(6),
                   height=1,
                   width=7)
  button6.grid(row=4, column=2)

  button7 = Button(gui,
                   text=' 7 ',
                   fg='white',
                   bg='silver',
                   command=lambda: insert(7),
                   height=1,
                   width=7)
  button7.grid(row=3, column=0)

  button8 = Button(gui,
                   text=' 8 ',
                   fg='white',
                   bg='silver',
                   command=lambda: insert(8),
                   height=1,
                   width=7)
  button8.grid(row=3, column=1)

  button9 = Button(gui,
                   text=' 9 ',
                   fg='white',
                   bg='silver',
                   command=lambda: insert(9),
                   height=1,
                   width=7)
  button9.grid(row=3, column=2)

  button0 = Button(gui,
                   text=' 0 ',
                   fg='white',
                   bg='silver',
                   command=lambda: insert(0),
                   height=1,
                   width=7)
  button0.grid(row=6, column=0)

  plus = Button(gui,
                text=' + ',
                fg='white',
                bg='silver',
                command=lambda: insert("+"),
                height=1,
                width=18)
  plus.grid(row=6, column=2, columnspan=2)

  minus = Button(gui,
                 text=' - ',
                 fg='white',
                 bg='silver',
                 command=lambda: insert("-"),
                 height=1,
                 width=7)
  minus.grid(row=5, column=3)

  multiply = Button(gui,
                    text=' * ',
                    fg='white',
                    bg='silver',
                    command=lambda: insert("*"),
                    height=1,
                    width=7)
  multiply.grid(row=4, column=3)

  divide = Button(gui,
                  text=' / ',
                  fg='white',
                  bg='silver',
                  command=lambda: insert("/"),
                  height=1,
                  width=7)
  divide.grid(row=3, column=3)

  equal = Button(gui,
                 text=' = ',
                 fg='white',
                 bg='silver',
                 command=equal_insert,
                 height=1,
                 width=18)
  equal.grid(row=2, column=0, columnspan=2)
  clear = Button(gui,
                 text='Clear',
                 fg='white',
                 bg='silver',
                 command=clear,
                 height=1,
                 width=18)
  clear.grid(row=2, column=2, columnspan=2)

  Decimal = Button(gui,
                   text='.',
                   fg='white',
                   bg='silver',
                   command=lambda: insert('.'),
                   height=1,
                   width=7)
  Decimal.grid(row=6, column=1)

  gui.mainloop()
